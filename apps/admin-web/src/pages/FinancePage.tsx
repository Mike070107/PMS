  ...
  return <div className="finance-page">
    ...
    <Tabs className="finance-tabs" items={[
      ...
      {key:'invoices',label:<span><InboxOutlined/> 发票收件箱 <Tag color="blue">{dashboard?.invoiceInbox||0}</Tag></span>,children:<Card className="finance-panel" title="发票收件箱" extra={<Space><Upload accept=".pdf,.ofd,.xml,.jpg,.jpeg,.png,.webp,.zip" showUploadList={false} customRequest={async({file,onSuccess,onError})=>{try{await uploadObject(file as File,'/finance/invoices/upload');onSuccess?.({});message.success('发票已导入');await load();}catch(e){onError?.(e as Error);message.error(errorText(e));}}}><Button icon={<FileAddOutlined/>}>上传发票</Button></Upload><Button icon={<CloudSyncOutlined/>} disabled={!mailStatus} onClick={syncMail}>同步 QQ 邮箱</Button></Space>}>
        <Alert className="mail-status" showIcon type={mailStatus?.lastError?'warning':'info'} message={mailStatus?`已连接 ${mailStatus.emailMasked}，首次同步最近 7 天`:'尚未连接 QQ 邮箱'} description={mailStatus?.lastError?`上次同步失败：${mailStatus.lastError}`:mailStatus?.lastSuccessAt?`上次成功：${dayjs(mailStatus.lastSuccessAt).format('YYYY-MM-DD HH:mm')}`:'连接后可自动收取新发票；不会标记已读、移动或删除邮件。'} action={<Button size="small" onClick={()=>setMailOpen(true)}>{mailStatus?'修改':'立即连接'}</Button>}/>
    ...
    <Modal title="连接 QQ 邮箱" open={mailOpen} onCancel={()=>setMailOpen(false)} footer={null} destroyOnClose><Alert showIcon type="info" message="使用 IMAP 授权码，不是 QQ 密码" description="系统只读同步收件箱，不会标记已读、移动或删除邮件。首次同步最近 7 天，之后只拉取新邮件。"/><Form form={mailForm} layout="vertical" onFinish={saveMail} initialValues={{enabled:true}}><Form.Item label="QQ 邮箱" name="email" rules={[{required:!mailStatus,message:'请填写 QQ 邮箱'}]}><Input placeholder={mailStatus?.emailMasked||'例如：123456@qq.com'} autoComplete="off"/></Form.Item><Form.Item label={mailStatus?'新授权码（留空保持不变）':'IMAP 授权码'} name="authorizationCode" rules={[{required:!mailStatus,message:'请填写 IMAP 授权码'}]}><Input.Password autoComplete="new-password"/></Form.Item><Form.Item name="enabled" valuePropName="checked"><Checkbox>启用自动同步</Checkbox></Form.Item><div className="modal-actions"><Button onClick={testMail}>测试连接</Button><Button type="primary" htmlType="submit">安全保存</Button></div></Form></Modal>
