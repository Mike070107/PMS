...
type Invoice = { id:number; originalName:string; source:string; amount:string|null; taxAmount?:string|null; invoiceDate:string|null; invoiceNo?:string|null; sellerName?:string|null; status:string; entryId:number|null; createdAt:string; discardReason?:string; recognitionStatus?:string; recognitionSource?:string|null; verificationStatus?:string; verifiedAt?:string|null; verificationSource?:string|null; verificationMessage?:string|null };
type VerificationHistory = { id:number; status:string; source:string; checkedAt:string; message?:string|null };
type TaxImportBatch = { id:number; originalName:string; status:string; totalRows:number; importedRows:number; updatedRows:number; failedRows:number; errors:Array<{row:number;message:string}>; createdAt:string };
...
type Attachment = { name:string; objectKey:string; contentType?:string; size?:number; previewUrl?:string };
...
const verificationLabels: Record<string,[string,string]> = { unverified:['未验真','default'], pending:['查验中','blue'], verified:['验真通过','green'], failed:['验真不通过','red'], exception:['查验异常','orange'] };
const recognitionLabels: Record<string,[string,string]> = { pending:['待识别','default'], recognized:['已识别','green'], partial:['部分识别','orange'], failed:['识别失败','red'] };
...
function isImageFile(file: Pick<File,'name'|'type'> | Attachment) { return /^image\//i.test('type' in file ? file.type : file.contentType || '') || /\.(png|jpe?g|gif|webp|bmp)$/i.test(file.name); }
...
export default function FinancePage() {
  ...
  const [entryOpen,setEntryOpen]=useState(false); const [projectOpen,setProjectOpen]=useState(false); const [mailOpen,setMailOpen]=useState(false); const [reimburseOpen,setReimburseOpen]=useState(false); const [taxImportOpen,setTaxImportOpen]=useState(false);
  ...
  const [verificationInvoice,setVerificationInvoice]=useState<Invoice|null>(null); const [verificationHistory,setVerificationHistory]=useState<VerificationHistory[]>([]); const [taxBatches,setTaxBatches]=useState<TaxImportBatch[]>([]);
  ...
  useEffect(()=>()=>{ previewUrls.current.forEach(url=>URL.revokeObjectURL(url)); previewUrls.current.clear(); },[]);
  ...
  const addAttachment=async(file:File,target:'project'|'voucher')=>{let previewUrl:string|undefined;try{const saved=await uploadObject(file);if(target==='voucher'&&isImageFile(file)){previewUrl=URL.createObjectURL(file);previewUrls.current.add(previewUrl);}const attachment:Attachment={...saved,name:saved.name||file.name,contentType:saved.contentType||file.type,previewUrl};if(target==='project')setProjectAttachments(list=>[...list,attachment]);else setVoucherAttachments(list=>[...list,attachment]);message.success(`${file.name||'截图'}已上传`);}catch(e){if(previewUrl){URL.revokeObjectURL(previewUrl);previewUrls.current.delete(previewUrl);}message.error(errorText(e));throw e;}};
  ...
  const clearVoucherAttachments=()=>{voucherAttachments.forEach(file=>{if(file.previewUrl){URL.revokeObjectURL(file.previewUrl);previewUrls.current.delete(file.previewUrl);}});setVoucherAttachments([]);};
  const removeVoucherAttachment=(index:number)=>setVoucherAttachments(list=>{const file=list[index];if(file?.previewUrl){URL.revokeObjectURL(file.previewUrl);previewUrls.current.delete(file.previewUrl);}return list.filter((_,itemIndex)=>itemIndex!==index);});
  const closeEntry=()=>{setEntryOpen(false);entryForm.resetFields();clearVoucherAttachments();};
  const saveEntry=async(values:any)=>{try{const voucherPayload=voucherAttachments.map(({previewUrl:_,...file})=>file);await request({method:'POST',url:'/finance/entries',data:{...values,businessDate:values.businessDate?.format('YYYY-MM-DD'),voucherAttachments:voucherPayload}});message.success('流水已保存');setEntryOpen(false);entryForm.resetFields();clearVoucherAttachments();await load();}catch(e){message.error(errorText(e));}};
  ...
  const importTaxAccount=async(file:File)=>{try{const result:any=await uploadObject(file,'/finance/tax-account-imports');message.success(`导入完成：新增 ${result.importedRows} 张，更新 ${result.updatedRows} 张${result.failedRows?`，失败 ${result.failedRows} 行`:''}`);setTaxBatches(await request({url:'/finance/tax-account-imports'}) as TaxImportBatch[]);await load();return result;}catch(e){message.error(errorText(e));throw e;}};
  const openTaxImport=async()=>{setTaxImportOpen(true);try{setTaxBatches(await request({url:'/finance/tax-account-imports'}) as TaxImportBatch[]);}catch(e){message.error(errorText(e));}};
  const openVerificationHistory=async(invoice:Invoice)=>{setVerificationInvoice(invoice);try{setVerificationHistory(await request({url:`/finance/invoices/${invoice.id}/verification-history`}) as VerificationHistory[]);}catch(e){message.error(errorText(e));}};
  ...
  const invoiceColumns=[
    {title:'收到时间',dataIndex:'createdAt',render:(v:string)=>dayjs(v).format('YYYY-MM-DD HH:mm')},{title:'发票',dataIndex:'originalName',minWidth:240,render:(v:string,r:Invoice)=><div><strong>{r.invoiceNo||v}</strong>{r.sellerName&&<div className="invoice-secondary">{r.sellerName}</div>}</div>},
    {title:'金额',dataIndex:'amount',render:(v:string|null)=>v?`¥${money(v)}`:'待识别'},{title:'来源',dataIndex:'source',render:(v:string)=>v==='email'?'QQ 邮箱':v==='tax_account'?'税务数字账户':'手工上传'},
    {title:'识别',dataIndex:'recognitionStatus',render:(v:string='pending')=>{const x=recognitionLabels[v]||[v,'default'];return <Tag color={x[1]}>{x[0]}</Tag>}},
    {title:'验真',dataIndex:'verificationStatus',render:(v:string='unverified',r:Invoice)=>{const x=verificationLabels[v]||[v,'default'];return <button type="button" className="invoice-status-button" onClick={()=>openVerificationHistory(r)}><Tag color={x[1]}>{x[0]}</Tag>{r.verifiedAt&&<span>{dayjs(r.verifiedAt).format('MM-DD HH:mm')}</span>}</button>}},
    {title:'状态',dataIndex:'status',render:(v:string)=>{const x=invoiceLabels[v]||[v,'default'];return <Tag color={x[1]}>{x[0]}</Tag>}},
    {title:'操作',key:'actions',render:(_:unknown,r:Invoice)=><Space>{r.status==='inbox'&&<><Button type="primary" size="small" onClick={()=>openMatch(r)}>匹配流水</Button><Button size="small" onClick={()=>discard(r)}>丢弃</Button></>}{r.status==='discarded'&&<Button size="small" onClick={async()=>{await request({method:'POST',url:`/finance/invoices/${r.id}/restore`});await load();}}>恢复</Button>}<Button size="small" icon={<HistoryOutlined/>} onClick={()=>openVerificationHistory(r)}>记录</Button></Space>},
  ];
  ...
  return <div className="finance-page">
    ...
    <Tabs className="finance-tabs" items={[
      ...
      {key:'invoices',label:<span><InboxOutlined/> 发票收件箱 <Tag color="blue">{dashboard?.invoiceInbox||0}</Tag></span>,children:<Card className="finance-panel" title="发票收件箱" extra={<Space wrap><Button icon={<FileExcelOutlined/>} onClick={openTaxImport}>税务数字账户导入</Button><Upload accept=".pdf,.ofd,.xml,.jpg,.jpeg,.png,.webp" showUploadList={false} customRequest={async({file,onSuccess,onError})=>{try{await uploadObject(file as File,'/finance/invoices/upload');onSuccess?.({});message.success('发票已导入并完成识别');await load();}catch(e){onError?.(e as Error);message.error(errorText(e));}}}><Button icon={<FileAddOutlined/>}>上传发票</Button></Upload><Button icon={<CloudSyncOutlined/>} disabled={!mailStatus} onClick={syncMail}>同步 QQ 邮箱</Button></Space>}>
        ...
        <Table rowKey="id" columns={invoiceColumns} dataSource={invoices} scroll={{x:1320}} pagination={{pageSize:20}} locale={{emptyText:<Empty description="暂未收到发票"/>}}/>
      </Card>},
    ...
    <Modal title="新增记账流水" open={entryOpen} onCancel={closeEntry} footer={null} width={760} destroyOnClose><Form form={entryForm} layout="vertical" onFinish={saveEntry} preserve><div className="form-grid"><Form.Item label="日期" name="businessDate" rules={[{required:true,message:'请选择日期'}]}><DatePicker/></Form.Item><Form.Item label="收入 / 支出" name="flowType" rules={[{required:true,message:'请选择方向'}]}><Radio.Group buttonStyle="solid"><Radio.Button value="expense">支出</Radio.Button><Radio.Button value="income">收入</Radio.Button></Radio.Group></Form.Item><Form.Item label="金额" name="amount" rules={[{required:true,message:'请填写金额'}]}><InputNumber min={0.01} precision={2} prefix="¥" className="full-width"/></Form.Item><Form.Item label="归属" name="owner" rules={[{required:true,message:'请选择归属'}]}><Radio.Group><Radio value="osiris">OsirisList</Radio><Radio value="pruis">普睿斯</Radio><Radio value="personal">个人</Radio></Radio.Group></Form.Item><Form.Item className="span-2" label="事由" name="reason" rules={[{required:true,message:'请填写事由'}]}><Input.TextArea rows={3} maxLength={500} showCount placeholder="例如：购买项目现场用的照明材料"/></Form.Item><Form.Item label="项目（可后补）" name="projectId"><Select allowClear options={roots.map(p=>({value:p.id,label:p.name}))}/></Form.Item><Form.Item label="子项目（可后补）" name="subProjectId"><Select allowClear options={projects.filter(p=>p.parentId).map(p=>({value:p.id,label:`${projectName(p.parentId)} / ${p.name}`}))}/></Form.Item><Form.Item label="支付方式" name="paymentMethod"><Radio.Group><Radio value="wechat">微信</Radio><Radio value="alipay">支付宝</Radio><Radio value="bank">对公转账</Radio><Radio value="cash">现金</Radio></Radio.Group></Form.Item><Form.Item label="需要报销" name="reimbursementRequired" valuePropName="checked"><Checkbox>保存后进入待报销清单</Checkbox></Form.Item><Form.Item className="span-2" label="凭证">
      <div className="voucher-upload-zone" onPaste={pasteAttachment('voucher')}>
        <Upload.Dragger
          className={`voucher-drop-surface ${voucherAttachments.length ? 'voucher-drop-surface--filled' : ''}`}
          multiple
          showUploadList={false}
          openFileDialogOnClick={false}
          customRequest={async({file,onSuccess,onError})=>{try{await addAttachment(file as File,'voucher');onSuccess?.({});}catch(e){onError?.(e as Error);}}}
        >
          {voucherAttachments.length === 0 ? <>
            <p className="ant-upload-drag-icon"><InboxOutlined/></p>
            <p>拖拽凭证到这里，或直接粘贴截图</p>
            <p className="upload-hint">图片会直接显示在这个虚线框内，点击即可预览</p>
          </> : <Image.PreviewGroup>
            <div className="voucher-preview-grid">
              {voucherAttachments.map((file,index)=><div className={`voucher-preview-card ${isImageFile(file)?'voucher-preview-card--image':'voucher-preview-card--file'}`} key={file.objectKey}>
                {isImageFile(file)&&file.previewUrl?<Image src={file.previewUrl} alt={file.name} preview={{mask:'点击预览'}}/>:<div className="voucher-file-card"><FileOutlined/><span>{file.name}</span></div>}
                <button type="button" className="voucher-remove" aria-label={`删除 ${file.name}`} onClick={(event)=>{event.stopPropagation();removeVoucherAttachment(index)}}><DeleteOutlined/></button>
                <div className="voucher-preview-name" title={file.name}>{file.name}</div>
              </div>)}
            </div>
          </Image.PreviewGroup>}
        </Upload.Dragger>
        <div className="voucher-upload-actions">
          <Upload multiple showUploadList={false} customRequest={async({file,onSuccess,onError})=>{try{await addAttachment(file as File,'voucher');onSuccess?.({});}catch(e){onError?.(e as Error);}}}><Button icon={<FileAddOutlined/>}>选择凭证文件</Button></Upload>
          <span>已上传 {voucherAttachments.length} 个，单个不超过 30MB</span>
        </div>
      </div>
    </Form.Item></div><div className="modal-actions"><Button onClick={closeEntry}>取消</Button><Button type="primary" htmlType="submit">保存流水</Button></div></Form></Modal>
    ...
    <Modal title="导入税务数字账户发票" open={taxImportOpen} onCancel={()=>setTaxImportOpen(false)} footer={null} width={760}>
      <Alert showIcon type="info" message="从税务数字账户导出后直接上传" description="进入电子税务局 → 税务数字账户 → 发票查询统计 → 全量发票查询，分别选择“开具发票”或“取得发票”并导出。请保留原始列名，不要合并单元格。"/>
      <Upload.Dragger className="tax-import-drop" accept=".xlsx,.xls,.csv" maxCount={1} showUploadList={false} customRequest={async({file,onSuccess,onError})=>{try{const result=await importTaxAccount(file as File);onSuccess?.(result);}catch(e){onError?.(e as Error);}}}>
        <p className="ant-upload-drag-icon"><FileExcelOutlined/></p><p>拖入税务数字账户导出的 XLSX、XLS 或 CSV</p><p className="upload-hint">系统按发票代码与号码去重；再次导入会补全字段和验真结果，不会重复建票。</p>
      </Upload.Dragger>
      <div className="tax-import-history"><h3>最近导入</h3>{taxBatches.length?<Table size="small" rowKey="id" pagination={false} scroll={{x:620}} dataSource={taxBatches.slice(0,8)} columns={[{title:'时间',dataIndex:'createdAt',render:(v:string)=>dayjs(v).format('MM-DD HH:mm')},{title:'文件',dataIndex:'originalName',ellipsis:true},{title:'新增',dataIndex:'importedRows'},{title:'更新',dataIndex:'updatedRows'},{title:'失败',dataIndex:'failedRows',render:(v:number)=><span className={v?'tax-import-error':''}>{v}</span>},{title:'状态',dataIndex:'status',render:(v:string)=><Tag color={v==='completed'?'green':v==='partial'?'orange':'red'}>{v==='completed'?'完成':v==='partial'?'部分完成':'失败'}</Tag>}]} />:<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="还没有导入记录"/>}</div>
    </Modal>
    <Modal title="发票验真记录" open={!!verificationInvoice} onCancel={()=>setVerificationInvoice(null)} footer={<Button onClick={()=>setVerificationInvoice(null)}>关闭</Button>} width={680}>
      {verificationInvoice&&<div className="verification-summary"><div><span>发票</span><strong>{verificationInvoice.invoiceNo||verificationInvoice.originalName}</strong></div><div><span>当前状态</span>{(()=>{const x=verificationLabels[verificationInvoice.verificationStatus||'unverified'];return <Tag color={x?.[1]}>{x?.[0]}</Tag>})()}</div><div><span>查验来源</span><strong>{verificationInvoice.verificationSource||'—'}</strong></div><div><span>最近查验</span><strong>{verificationInvoice.verifiedAt?dayjs(verificationInvoice.verifiedAt).format('YYYY-MM-DD HH:mm'):'—'}</strong></div></div>}
      <Alert showIcon type="info" message="统一验真接口已预留" description="当前先接收税务数字账户导入的查验结果。接入乐企或合规服务商后，每次在线查验也会写入同一份历史记录。"/>
      <div className="verification-timeline">{verificationHistory.length?verificationHistory.map(item=>{const x=verificationLabels[item.status]||[item.status,'default'];return <div className="verification-event" key={item.id}><div><Tag color={x[1]}>{x[0]}</Tag><strong>{item.source}</strong><time>{dayjs(item.checkedAt).format('YYYY-MM-DD HH:mm:ss')}</time></div>{item.message&&<p>{item.message}</p>}</div>}):<Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="尚无验真历史；导入含查验结果的税务数字账户文件后会自动记录"/>}</div>
    </Modal>
