...
const SYNONYM_GROUPS: string[][] = [
  ...
  ['门铃', '按铃', '呼叫器', '门铃不响'],
  // 「摄像」一个词就盖住摄像头 / 摄像机；「图像没有」「没图像」是监控坏了最常见的说法
  // （2026-09-07 Mike 报「有 5 个摄像头图像没有」落到了「其它」）
  ['监控', '摄像头', '摄像机', '摄像', '探头', '录像', '录像机', '硬盘录像机', '球机', '枪机',
    '没图像', '无图像', '图像没有', '没有图像', '监控室'],
  ['道闸', '抬杆', '起杆', '落杆', '闸机', '挡杆', '车牌识别', '车牌', '停车系统'],
  // 电梯
  ['电梯', '轿厢', '困人', '梯门', '楼层显示'],
...
export function buildTypeKeywords(rule: {
  ...
  // 给一组稳定锚点，把弱电的五个同义词组（门铃 / 门禁 / 对讲 / 监控 / 道闸）全带进来；
  // 租户自建的「门铃/对讲…」类型本来就会由标题切出来。
  // 2026-09-07 之前只锚了「门铃」：吴泾的「智能化相关」配的常用短语里没有一个监控词，
  // 「有 5 个摄像头图像没有」一个字都没撞上，落到了「其它」。
  if (/智能|弱电/.test(rule.label)) {
    for (const anchor of ['门铃', '门禁', '对讲', '监控', '道闸']) {
      if (!seeds.includes(anchor)) seeds.push(anchor);
    }
  }
