...
export class RolesService {
  ...
  private toOption(r: Role) {
    return { id: r.id, name: r.name, builtIn: r.builtIn, dataScope: r.dataScope };
  }
  ...
  private async writePermsAndScopes(
    ...
    const allowed = access.enabledPages
      ? ADMIN_PAGE_KEYS.filter((k) => access.enabledPages!.includes(k))
      : [...ADMIN_PAGE_KEYS];
    const byPage = new Map<string, { canView: boolean; canEdit: boolean; canDelete: boolean }>();
    for (const p of dto.permissions) {
      if (!allowed.includes(p.pageKey as (typeof allowed)[number])) continue;
