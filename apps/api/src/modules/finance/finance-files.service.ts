...
import { extractInvoiceHints, isInvoiceAttachment, sha256 } from './finance.util';
...
export class FinanceFilesService {
  ...
  async uploadInvoice(user: AuthUser, file?: Express.Multer.File) {
    ...
    const hints = extractInvoiceHints(file.originalname, file.buffer);
    ...
    const invoice = await this.invoiceRepo.save(this.invoiceRepo.create({ tenantId, source: 'upload', originalName: file.originalname.slice(0, 255), objectKey: stored.objectKey, sha256: hash, status: sameIdentity ? 'duplicate' : 'inbox', ...hints, sellerName: null, buyerName: null, entryId: null, sourceMessageId: null, matchReason: sameIdentity ? `与发票 #${sameIdentity.id} 的代码和号码重复` : null, discardReason: null, recognitionRaw: null, createdBy: user.id, updatedBy: user.id }));
