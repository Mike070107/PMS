...
from deploy_config import config as TestServerConfig
...
# 加载测试服务器配置
app.config.from_object(TestServerConfig)

# 安全密钥配置（使用测试服务器配置）
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', TestServerConfig.SECRET_KEY)
app.config['JWT_SECRET_KEY'] = os.environ.get('JWT_SECRET_KEY', TestServerConfig.JWT_SECRET_KEY)
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("公寓物业收费系统 - 后端服务启动 (测试服务器)")
    print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"数据库: {app.config.get('SQLALCHEMY_DATABASE_URI', '未配置')}")
    print(f"服务地址: http://{host}:{port}")
    print(f"测试页面: http://192.168.1.251:{port}/test")
    print("=" * 50)
