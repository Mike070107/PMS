...
Windows 上开发者工具 CLI 内置代理默认监听 3799；如果 Hyper-V/WinNAT 把该端口段列为系统保留，
原生 CLI 会报 `listen EACCES` 。`tools/miniapp-ship.mjs` 会只对 CLI 子进程把该监听转到 47890，
不修改系统保留端口。中文 CLI 输出的 `×` 也按失败处理，不得写入成功发布台账。
