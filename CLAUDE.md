...
- 用户说“部署”“推送上线”时，默认完成所有受影响端的完整发布闭环：构建与校验、推送 `main`、
  发布 API/后台、上传小程序、把新版本设为体验版，并在真实线上入口验证版本号与关键功能。
  **“上传开发版本”不算完成**；未确认公众平台的体验版已经指向本次 hash，不得回复“部署完成”。
  能自动完成的平台操作不要拆成步骤交给用户。若登录、验证码或平台安全策略确实阻塞自动操作，
  必须在当次发布中明确说明唯一阻塞点，不能先宣称完成再等用户发现。
