...
with app.app_context():
    try:
        # 检查表是否存在
        result = db.session.execute(db.text("SHOW TABLES LIKE 'operation_log'")).fetchone()
        
        if result:
            print("✓ operation_log表已存在")
            
            # 检查表结构
            cols = db.session.execute(db.text("DESCRIBE operation_log")).fetchall()
            print(f"✓ 表包含 {len(cols)} 个字段")
            
            # 检查是否有数据
            count = db.session.execute(db.text("SELECT COUNT(*) FROM operation_log")).scalar()
            ...
            print("✗ operation_log表不存在，需要执行SQL脚本创建")
            print("\n请执行以下命令创建表:")
