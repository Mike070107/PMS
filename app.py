...
class OperationLog(db.Model):
    ...
    # 用户信息
    用户ID = db.Column('用户ID', db.Integer, nullable=False, index=True, comment='操作用户ID')
    用户账号 = db.Column('用户账号', db.String(50), nullable=False, index=True, comment='操作用户的USERNAME')
    用户姓名 = db.Column('用户姓名', db.String(50), comment='用户真实姓名')
    用户角色 = db.Column('用户角色', db.String(20), nullable=False, comment='用户角色')
    所属小区 = db.Column('所属小区', db.String(100), index=True, comment='所属小区')
    小区编号 = db.Column('小区编号', db.Integer, index=True, comment='小区编号')
    
    # 网络信息
    电脑IP = db.Column('电脑IP', db.String(50), comment='IP地址')
    MAC地址 = db.Column('MAC地址', db.String(50), comment='MAC地址')
    用户代理 = db.Column('用户代理', db.Text, comment='浏览器UserAgent')
    
    # 操作信息
    操作类型 = db.Column('操作类型', db.String(50), nullable=False, index=True, comment='操作类型')
    操作模块 = db.Column('操作模块', db.String(50), nullable=False, index=True, comment='操作模块')
    操作详情 = db.Column('操作详情', db.Text, comment='操作详情（JSON格式）')
    
    # 业务相关
    目标ID = db.Column('目标ID', db.String(100), index=True, comment='操作对象ID')
    目标类型 = db.Column('目标类型', db.String(50), comment='操作对象类型')
    
    # 结果信息
    操作结果 = db.Column('操作结果', db.String(20), nullable=False, default='success', comment='操作结果')
    错误信息 = db.Column('错误信息', db.Text, comment='错误信息')
    
    # 请求信息
    请求方法 = db.Column('请求方法', db.String(10), comment='请求方法')
    请求URL = db.Column('请求URL', db.Text, comment='请求URL')
    请求参数 = db.Column('请求参数', db.Text, comment='请求参数')
    
    # 响应信息
    响应时间 = db.Column('响应时间', db.Integer, comment='响应时间（毫秒）')
