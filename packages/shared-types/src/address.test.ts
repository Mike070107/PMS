...
import { formatAddressLine, formatReporterRoomLabel, isLaneRedundant, formatRoomText } from './address';
...
test('地址行里的名字房号也不缀「室」', () => {
  assert.equal(
    formatAddressLine({ name: '吴泾物业总公司', mainLane: null, laneCount: 0 }, { lane: null, buildingNo: '858', roadName: null }, '工程部'),
    '吴泾物业总公司858号工程部',
  );
});
